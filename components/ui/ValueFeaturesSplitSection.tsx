"use client";

import Link from "next/link";

import { ParallaxCoverImage } from "@/components/ui/ParallaxCoverImage";

export type ValueFeatureItem = {
  title: string;
  description: string;
};

export type ValueFeaturesSplitSectionProps = {
  sectionId?: string;
  imageSrc: string;
  imageAlt: string;
  headingLead: string;
  headingRest: string;
  intro?: string;
  features: readonly ValueFeatureItem[];
  ctaHref?: string;
  ctaLabel?: string;
};

function FeatureRow({ title, description }: ValueFeatureItem) {
  return (
    <div className="flex flex-col gap-5">
      <div className="h-px w-full bg-[#263cd0]/16" />
      <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2">
        <span aria-hidden className="row-start-1 size-2 shrink-0 self-center rounded-full bg-[#263cd0]" />
        <h3 className="col-start-2 row-start-1 text-[22px] font-semibold leading-[1.2] tracking-[-0.02em] text-[#16216b]">
          {title}
        </h3>
        <p className="col-start-2 row-start-2 text-[16px] leading-relaxed text-[#16216b]">{description}</p>
      </div>
    </div>
  );
}

export function ValueFeaturesSplitSection({
  sectionId,
  imageSrc,
  imageAlt,
  headingLead,
  headingRest,
  intro,
  features,
  ctaHref = "/kontaktai#uzklausa",
  ctaLabel = "Gauti pasiūlymą",
}: ValueFeaturesSplitSectionProps) {
  return (
    <section className="relative z-[2] w-full bg-white py-16 md:py-[100px]" id={sectionId}>
      <div className="mx-auto w-full max-w-[1440px] px-4 md:px-[70px]">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-10 xl:gap-14">
          <div className="relative min-h-[360px] overflow-hidden rounded-2xl bg-[#e8ebfa] lg:min-h-[840px]">
            <ParallaxCoverImage
              alt={imageAlt}
              fill
              loading="lazy"
              sizes="(max-width: 1023px) 100vw, 46vw"
              src={imageSrc}
            />
          </div>

          <div className="flex max-w-[560px] flex-col gap-12 lg:gap-16 xl:gap-20">
            <div className="flex flex-col gap-5">
              <h2 className="text-3xl font-semibold leading-[1.12] tracking-[-0.03em] text-[#16216b] md:text-[45px] md:leading-[52px]">
                <span className="text-[#263cd0]">{headingLead}</span>
                <span className="text-[#16216b]">{headingRest}</span>
              </h2>
              {intro ? (
                <p className="text-base leading-relaxed text-[#16216b] md:text-[17px]">{intro}</p>
              ) : null}
            </div>

            <div className="flex flex-col gap-6">
              {features.map((feature) => (
                <FeatureRow description={feature.description} key={feature.title} title={feature.title} />
              ))}
              <div className="h-px w-full bg-[#263cd0]/16" />
            </div>

            <div>
              <Link
                className="inline-flex items-center justify-center rounded-full bg-[#263cd0] px-8 py-[15px] text-[15px] font-semibold text-white transition hover:bg-[#1e31a8]"
                href={ctaHref}
              >
                {ctaLabel}
              </Link>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
